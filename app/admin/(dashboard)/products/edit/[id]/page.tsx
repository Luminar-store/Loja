...
import { ArrowLeft, UploadCloud, Save, Loader2, Star, Trash2, X, ChevronLeft, ChevronRight, ArrowLeftRight } from 'lucide-react';
...
export default function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
      ...
      try {
        ...
        if (prod) {
          setFormData({
            ...
            material: prod.material || 'Ouro Amarelo',
            weight: prod.weight,
            width: prod.width,
            height: prod.height,
          ...
          // Inicializa a galeria com as imagens retornadas de product_images
          const loadedMedia: ProductMedia[] = (prod.images || []).map((imgUrl, idx) => ({
            url: imgUrl,
            position: idx,
            is_primary: imgUrl === prod.image_url,
            uploading: false
          }));
  ...
  // Upload múltiplo de mídias com compressão WebP
          ...
          setMediaList(prev => {
            ...
            if (targetIdx !== -1) {
              updated[targetIdx] = {
                url,
                position: targetIdx,
                is_primary: updated.length === files.length && targetIdx === 0 ? true : false,
                uploading: false
              };
            }
  ...
  const handleSave = async () => {
    ...
    if (!hasPrimary) {
      toast.error('Defina uma imagem de destaque clicando na estrela.');
      return;
    }
    ...
    try {
      ...
      console.error(error);
      toast.error('Erro ao salvar as edições do produto.');
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="space-y-6 max-w-5xl mx-auto pb-20">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-white/50 text-sm mt-1">Atualize as informações e ordene as imagens da joia.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          ...
          {/* Media Manager (Múltiplas Imagens) */}
          <motion.div 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
                        ...
                        <div className="absolute inset-0 bg-black/70 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-between p-3 z-10">
                          <div className="flex justify-between items-center">
                            <button 
                              ...
                              title={item.is_primary ? 'Imagem Destaque' : 'Definir Destaque'}
                            >
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <input name="promotional_price" value={formData.promotional_price || ''} onChange={handleChange} type="number" step="0.01" className="w-full bg-[#1A1A1A] border border-white/10 rounded-lg py-2.5 px-4 text-white focus:border-[#d4af37] outline-none transition-colors" />
              </div>
              <div>
                <label className="block text-xs uppercase tracking-widest text-white/50 mb-2">Estoque (Unidades)</label>
                <input name="stock" value={formData.stock} onChange={handleChange} type="number" className="w-full bg-[#1A1A1A] border border-white/10 rounded-lg py-2.5 px-4 text-white focus:border-[#d4af37] outline-none transition-colors" />
              </div>
            </div>
          </motion.div>
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="col-span-2">
                ...
                <input name="weight" value={formData.weight || ''} onChange={handleChange} type="number" step="0.01" className="w-full bg-[#1A1A1A] border border-white/10 rounded-lg py-2.5 px-4 text-white focus:border-[#d4af37] outline-none transition-colors" />
              </div>
              <div>
                <label className="block text-xs uppercase tracking-widest text-white/50 mb-2">Largura (cm)</label>
                <input name="width" value={formData.width || ''} onChange={handleChange} type="number" step="0.1" className="w-full bg-[#1A1A1A] border border-white/10 rounded-lg py-2.5 px-4 text-white focus:border-[#d4af37] outline-none transition-colors" />
              </div>
              <div>
                <label className="block text-xs uppercase tracking-widest text-white/50 mb-2">Altura (cm)</label>
                <input name="height" value={formData.height || ''} onChange={handleChange} type="number" step="0.1" className="w-full bg-[#1A1A1A] border border-white/10 rounded-lg py-2.5 px-4 text-white focus:border-[#d4af37] outline-none transition-colors" />
              </div>
            </div>
          </motion.div>
        </div>
